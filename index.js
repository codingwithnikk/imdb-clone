...
(function () {
  ...
  const containerSuggestion = document.getElementById("card-container");
  const favContainer = document.getElementById("fav-movies-container");
  const favouritesList = document.getElementById("favorites-section");
  const favText = document.getElementById("empty-fav-text");
  ...
  searchText.addEventListener("keyup", function () {
    ...
    if (search === "") {
      ...
      containerSuggestion.style.display = "grid";
    }
  });

  // Fetches data from api and calls function to add it in
  async function fetchMovies(search) {
    const url = `https://www.omdbapi.com/?t=${search}&apikey=8129c097`;
    try {
  ...
  function addSuggestion(data) {
    ...
    `;
    containerSuggestion.prepend(movieCard);
    }
  }
  ...
  async function handleFavBtn(e) {
    ...
    if (favMoviesLocal) {
      favListArray = Array.from(JSON.parse(favMoviesLocal));
    } else {
    ...
    let isPresent = false;
    favListArray.forEach((movie) => {
      if (data.Title == movie.Title) {
        alert("already added to fav list");
        isPresent = true;
      }
    });

    if (!isPresent) {
      favListArray.push(data);
    }

    localStorage.setItem("favMoviesList", JSON.stringify(favListArray));
    isPresent = !isPresent;
    addToFav();
  }
  ...
  async function handleClickListner(e) {
    ...
    if (target.classList.contains("add-fav")) {
      ...
      if (favouritesList.style.display == "flex") {
